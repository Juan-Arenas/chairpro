...
export const demoBarbershop: Barbershop = {
  ...
  settings: {
    ...
    currency: 'COP',
    currencySymbol: '$',
    enableClientBot: true,
    enableBarberBot: true,
    enableGoogleMapsReview: true,
    googleMapsUrl: 'https://maps.google.com/?q=The+Black+Chair+Bogota',
    enableDailyCloseWhatsApp: true,
    dailyClosePhone: '+57 316 555 0190',
...
export const demoShops: Barbershop[] = [
  demoBarbershop,
  {
    ...
    settings: {
      ...
      enableBarberBot: true,
      enableGoogleMapsReview: true,
      googleMapsUrl: 'https://maps.google.com/?q=Fade+Master+Studio+Medellin',
      enableDailyCloseWhatsApp: true,
      dailyClosePhone: '+57 300 444 0220',
      enableReminders: true,
    },
    plan: 'enterprise',
  ...
  },
  {
    ...
    settings: {
      ...
      enableBarberBot: true,
      enableGoogleMapsReview: true,
      googleMapsUrl: 'https://maps.google.com/?q=La+Clasica+Barber+Club+Cali',
      enableDailyCloseWhatsApp: true,
      dailyClosePhone: '+57 315 333 0330',
      enableReminders: true,
    },
    plan: 'basic',
...
export const demoSaasPayments: SaasPayment[] = [
  {
    ...
    notes: 'Pago mensualidad puntual Nequi Plan MartiArenas Labs',
  ...
  },
  {
    ...
    notes: 'Transferencia Bancolombia Plan MartiArenas Labs',
    recordedBy: 'Juan Arenas (SuperAdmin)',
  ...
  },
  {
    ...
    notes: 'Pago Daviplata Plan MartiArenas Labs',
    recordedBy: 'Juan Arenas (SuperAdmin)',
...
export const demoAutomations: Automation[] = [
  {
    id: 'auto_barber_bot', shopId: 'shop_demo',
    name: 'Bot WhatsApp para Barberos (Registro & Estados)',
    trigger: 'on_appointment_completed',
    triggerLabel: 'Al recibir mensaje de barbero por WhatsApp ("Ocupado", "Corte a Carlos $35k")',
    steps: [
      { action: 'update_stats', label: 'Actualizar disponibilidad del barbero en tiempo real', delay: 0 },
      { action: 'register_income', label: 'Registrar venta y comisión automáticamente', delay: 0 },
      { action: 'send_confirmation', label: 'Enviar comprobante inmediato al barbero por WhatsApp', delay: 0 },
    ],
    isActive: true, runCount: 142, lastRunAt: today + 'T11:20:00Z',
  },
  {
    id: 'auto_client_bot', shopId: 'shop_demo',
    name: 'Agendamiento Autónomo 24/7 para Clientes (Bot WhatsApp)',
    trigger: 'on_appointment_created',
    triggerLabel: 'Al interactuar un cliente por WhatsApp para cotizar o agendar cita',
    steps: [
      { action: 'send_confirmation', label: 'Consultar disponibilidad real de barberos y calendario', delay: 0 },
      { action: 'create_notification', label: 'Agendar cita en el sistema y bloquear horario', delay: 0 },
      { action: 'send_confirmation', label: 'Confirmar reserva al cliente por WhatsApp con ubicación', delay: 0 },
    ],
    isActive: true, runCount: 96, lastRunAt: today + 'T10:45:00Z',
  },
  {
    id: 'auto_google_review', shopId: 'shop_demo',
    name: 'Reseña Post-Corte Google Maps (Anti-Spam 1 Sola Vez)',
    trigger: 'on_appointment_completed',
    triggerLabel: 'Al instante de que el barbero termina o registra el corte',
    steps: [
      { action: 'update_client_history', label: 'Verificar si el cliente ya recibió solicitud previa', delay: 0 },
      { action: 'send_confirmation', label: 'Enviar link de Google Maps por WhatsApp al instante (1 única vez)', delay: 0 },
      { action: 'create_notification', label: 'Registrar cliente como calificador contactado', delay: 0 },
    ],
    isActive: true, runCount: 68, lastRunAt: today + 'T11:00:00Z',
  },
  {
    id: 'auto_daily_close', shopId: 'shop_demo',
    name: 'Cierre de Caja Dinámico por WhatsApp',
    trigger: 'on_appointment_completed',
    triggerLabel: 'Automáticamente a la hora exacta de cierre de la barbería',
    steps: [
      { action: 'update_stats', label: 'Consolidar ingresos, métodos de pago y comisiones del día', delay: 0 },
      { action: 'send_confirmation', label: 'Enviar balance financiero completo al WhatsApp del dueño', delay: 0 },
    ],
    isActive: true, runCount: 30, lastRunAt: yesterday + 'T20:00:00Z',
  },
  {
